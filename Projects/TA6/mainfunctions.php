...
function validate(){
    ...
    try{
        ...
        $queryValidate = "SELECT id_user, first_name, email, password FROM TA6.Users WHERE email=:email";
...
function web_scrape($yr, $sem){
    ...
    if($fp1)
    {
        ...
        foreach($rows as $row)
        {
            ...
            $ta_needed = (intval($enroll_cap) / 10) + 1;
            $ta_registered = 0;

            // Insert data into Courses schema using BIND syntax
            $insertCourse = "
            INSERT IGNORE INTO TA6.Courses (year, semester, ta_needed, ta_registered, enroll_cap, enrolled, department, title, catalog_num)
    ...
    if($fp)
    {
        ...
        foreach($rows as $row)
        {
            ...
            if(strcmp($component, "Lecture") == 0 || strcmp($component, "Special Topics") == 0)
            {
                ...
                $insertCourseInfo = "
                    INSERT IGNORE INTO TA6.Course_Info (semester, year, catalog_num, department, instructor_full_name, component, units, time, location)
